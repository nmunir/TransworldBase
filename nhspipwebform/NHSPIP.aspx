...
<script runat="server">
    ...
    ' v1.3 - fixed issue where newly created users only had VIEW privilege
    
    ' TO DO
    ' alignment of fields in confirmation page
    
    Dim gsMode As String = "" ' uses DEMO account if set to "TEST", else live account
    ...
    Const EMAIL_ADDRESS_ALERTS As String = EMAIL_ADDRESS_SUPPORT & "," & EMAIL_ADDRESS_NHS & "," & EMAIL_ADDRESS_ACCOUNT_HANDLER
    ...
    Dim gbIsCreatingProductsAndAccounts As Boolean = True
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        If gsMode = "TEST" Then
            gsWebFormControlUserId = NHSPIPTEST_WEBFORM_CONTROL_USERID
            gnCustomerKey = NHSPIPTEST_CUSTOMER_KEY
        End If
    ...
    Protected Function BuildOrderEmailConfirmation(ByVal sType As String, ByVal nOrderNo As Integer) As String
        Dim sbOrderConfirmation As New StringBuilder
        sbOrderConfirmation.Append("Thank you for your order, which is now being processed as per the ordering guidance.")
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append("Your order may take up to 4 weeks to become available for draw down from the Sprint NHS PIP web site.")
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append("Please note the short code for your PCT/Trust is ")
        sbOrderConfirmation.Append(Highlight(ddlPCTs.SelectedValue))
        ...
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append("Order ref: #")
        sbOrderConfirmation.Append(nOrderNo.ToString)
        sbOrderConfirmation.Append(" received ")
        sbOrderConfirmation.Append(Now.ToString("dd-MMM-yyyy hh:mm"))
        sbOrderConfirmation.Append(" from ")
        ...
        sbOrderConfirmation.Append(ddlPCTs.SelectedItem.Text & " (" & ddlPCTs.SelectedValue & ")")
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append(NL(sType))
        If rbRequireAccount.Checked Then
            sbOrderConfirmation.Append("Your account user name is: ")
            sbOrderConfirmation.Append(Highlight(tbNewUserName.Text))
            sbOrderConfirmation.Append(" .")
            sbOrderConfirmation.Append(NL(sType))
            sbOrderConfirmation.Append("Your password is: ")
            sbOrderConfirmation.Append(Highlight(tbNewPassword.Text))
            sbOrderConfirmation.Append(" .")
        ElseIf rbHaveAccount.Checked Then
            sbOrderConfirmation.Append("Your account user name is: ")
            sbOrderConfirmation.Append(Highlight(tbUserName.Text))
            sbOrderConfirmation.Append(" .")
        Else
            sbOrderConfirmation.Append("You did not request any account action.")
        End If
        sbOrderConfirmation.Append(NL(sType))
        sbOrderConfirmation.Append(NL(sType))
                ...
                If tb.Text <> String.Empty Then
                    If IsNumeric(tb.Text) Then
                        ...
                        If nQty > 0 Then
                            ...
                            sbOrderConfirmation.Append(" - Qty: ")
    ...
    Protected Function GetOrCreateLinkedProductEntry(ByVal nGenericProductKey As Integer, ByVal nRingFencedProductKey As Integer) As Integer
        GetOrCreateLinkedProductEntry = 0
        Dim oDataTable As DataTable = ExecuteQueryToDataTable("SELECT BacklogQty FROM NHSPIPLinkedProducts WHERE GenericProductKey = " & nGenericProductKey & " AND RingFencedProductKey = " & nRingFencedProductKey)
        If oDataTable.Rows.Count > 0 Then
            GetOrCreateLinkedProductEntry = oDataTable.Rows(0).Item(0)
        Else
            Call ExecuteQueryToDataTable("INSERT INTO NHSPIPLinkedProducts (GenericProductKey, RingFencedProductKey, BacklogQty) VALUES (" & nGenericProductKey & "," & nRingFencedProductKey & ", 0)")
        End If
    End Function

    Protected Function ProcessProducts() As Integer
        Dim nOrderKey As Integer = CreateOrderRecord()
        Dim nLinkedProductKey As Integer
        Dim nQtyAvailable As Integer
        Dim nPreviousBacklogQty As Integer
        Dim sConsignmentNo As String = String.Empty
        ProcessProducts = nOrderKey
        Call Log(LOG_ENTRY_TYPE_ORDER, -1, nOrderKey, ddlPCTs.SelectedValue, "Assigned order # " & nOrderKey.ToString)
        For Each gvr As GridViewRow In gvProductInfo.Rows
            If gvr.RowType = DataControlRowType.DataRow Then
                Dim hidProductCode As HiddenField = gvr.FindControl("hidProductCode")
                'Dim tcProductCode As TableCell = gvr.Cells(0)
                Dim tcMaxGrab As TableCell = gvr.Cells(2)
                Dim tb As TextBox = gvr.FindControl("tbQty")
                If tb.Text.Trim <> String.Empty Then
                    ...
                    If nQtyRequested > 0 Then
                        ...
                        If gbIsCreatingProductsAndAccounts Then
                            Dim nThisOrderBacklogQty As Integer = 0
                            nQtyAvailable = GetQtyAvailableToPick(hidProductCode.Value, hidBaseProductKey.Value, nQtyRequested, nOrderKey, nOrderDetailKey)
                            nLinkedProductKey = GetLinkedProductKey(hidProductCode.Value, ddlPCTs.SelectedValue)
                            If Not nLinkedProductKey >= 0 Then
                                Call Log(LOG_ENTRY_TYPE_PRODUCT, hidBaseProductKey.Value, nOrderKey, ddlPCTs.SelectedValue, "Cloning product ~ " & hidProductCode.Value & " for " & ddlPCTs.SelectedValue)
                                nLinkedProductKey = AddNewProduct(hidBaseProductKey.Value, ddlPCTs.SelectedValue)
                                Call Log(LOG_ENTRY_TYPE_PRODUCT, nLinkedProductKey, nOrderKey, ddlPCTs.SelectedValue, "Created product ~ " & hidProductCode.Value & " | " & ddlPCTs.SelectedValue)
                                Call Log(LOG_ENTRY_TYPE_PRODUCT, nLinkedProductKey, nOrderKey, ddlPCTs.SelectedValue, "Permissioned " & PermissionProduct(nLinkedProductKey, ddlPCTs.SelectedValue) & " user(s) for product ~ " & hidProductCode.Value & " | " & ddlPCTs.SelectedValue)
                            End If
                            nPreviousBacklogQty = GetOrCreateLinkedProductEntry(hidBaseProductKey.Value, nLinkedProductKey)
                            If nQtyAvailable > 0 Then
                                ...
                                End If
                                nThisOrderBacklogQty = nQtyRequested - nQtyToPick
                                sConsignmentNo = GeneratePick(hidBaseProductKey.Value, nQtyToPick, "(" & nLinkedProductKey.ToString & ") Product code: " & hidProductCode.Value & " Product Date: " & ddlPCTs.SelectedValue)
                                If IsNumeric(sConsignmentNo) Then
                                    ...
                                    Call Log(LOG_ENTRY_TYPE_PRODUCT, hidBaseProductKey.Value, nOrderKey, ddlPCTs.SelectedValue, "Pick " & sConsignmentNo & " generated for " & nQtyToPick.ToString & " of ~ " & hidProductCode.Value & " | " & ddlPCTs.SelectedValue)
                                Else
                                    Dim sMessage As String = "Attempt to pick failed for ~ " & hidProductCode.Value & " | " & ddlPCTs.SelectedValue & " Consignment value returned: " & sConsignmentNo & " Order: " & nOrderKey & "/" & nOrderDetailKey.ToString
                                ...
                                End If
                            End If
                            If nThisOrderBacklogQty > 0 Then
                                Dim sMessage As String = "Insufficient pick quantity available (" & nQtyAvailable & ") for ring-fenced product (requested " & nQtyRequested & ") of ~ " & hidProductCode.Value & " | " & ddlPCTs.SelectedValue & " Order: " & nOrderKey & "/" & nOrderDetailKey.ToString
                                Call Log(LOG_ENTRY_TYPE_PRODUCT, hidBaseProductKey.Value, nOrderKey, ddlPCTs.SelectedValue, sMessage)
                            ...
                            End If
                            nPreviousBacklogQty = nPreviousBacklogQty + nThisOrderBacklogQty
                            Call ExecuteQueryToDataTable("UPDATE NHSPIPLinkedProducts SET BacklogQty = " & nPreviousBacklogQty & " WHERE GenericProductKey = " & hidBaseProductKey.Value & " AND RingFencedProductKey =  " & nLinkedProductKey)
                        End If
                    End If
                End If
            End If
    ...
    Protected Function GetLinkedProductKey(ByVal sProductCode As String, ByVal sProductDate As String) As Integer
        Dim oDataTable As DataTable
        Dim sSQL As String = "SELECT LogisticProductKey FROM LogisticProduct WHERE CustomerKey = " & gnCustomerKey & " AND ProductCode = '" & sProductCode.Replace("'", "''") & "' AND ISNULL(ProductDate,'') = '" & sProductDate.Replace("'", "''") & "'"
        Dim sMessage As String = "Checking if product ~ " & sProductCode & " | " & sProductDate & " exists: "
        oDataTable = ExecuteQueryToDataTable(sSQL)
        If oDataTable.Rows.Count = 0 Then
            GetLinkedProductKey = -1
            Call Log(LOG_ENTRY_TYPE_PRODUCT, -1, -1, sProductDate, sMessage & "NO")
        Else
            GetLinkedProductKey = oDataTable.Rows(0).Item(0)
            Call Log(LOG_ENTRY_TYPE_PRODUCT, GetLinkedProductKey, -1, sProductDate, sMessage & "YES - " & GetLinkedProductKey.ToString)
        End If
    End Function
...
<head id="Head1" runat="server">
    ...
    <title>NHS CFH SCR Public Information Programme - Order Materials (1.3)</title>
</head>
<body>
                    ...
                    <td colspan="2">
                        ...
                        <table style="width: 95%">
                            ...
                            <tr>
                                <td colspan="2">
                                    <fieldset>
                                        <legend>&nbsp;Access to your stock online&nbsp;</legend>
                                        <div class="fieldset_interior">
                            ...
                            <Columns>
                                ...
                                <asp:TemplateField HeaderText="Qty Required">
                                    <ItemTemplate>
                                        ...
                                        <asp:HiddenField ID="hidProductCode" runat="server"  Value='<%# Container.DataItem("ProductCode")%>' />
