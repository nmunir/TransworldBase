...
<script runat="server">
    ...
    Public Function ConvertDataTableToCSVString(ByVal oDataTable As DataTable) As String
        ...
        If sbResult.Length > 1 Then
            ...
            ' assume (a) there is at least one product used in the last n days, (b) product codes do not contain quote marks or commas
            For Each kv As KeyValuePair(Of String, String) In dictProductsUsed
        ...
        For Each oDataRow In oDataTable.Rows
            ...
            'Next oDataColumn
            'sbResult.Length = sbResult.Length - 1
            Call GetOrderDetails(oDataRow("ConsignmentKey"), dictProductRow)
    ...
    Protected Sub GetOrderDetails(ByVal sConsignmentNumber As String, ByRef dictProductsUsed As Dictionary(Of String, Integer))
        ...
        Try
            oConn.Open()
            oDataReader = oCmd.ExecuteReader()
            'For Each kv As KeyValuePair(Of String, Integer) In dictProductsUsed
            ' dictProductsUsed(kv.Key) = 0
            ' Next
...
<body>
    ...
    <div>
        <asp:Button ID="btnExport" runat="server" OnClick="btnExport_Click" Text="export last 365 days data to excel" />
