...
<script runat="server">
    ...
    Protected Sub PopulateItemGridView()
        Dim oDataTableOrDataView As New Object
        If rbTotals.Checked Then
            oDataTableOrDataView = dvGetProductTotals()
        ElseIf rbConsignments.Checked Then
            oDataTableOrDataView = dtGetConsignmentBreakdown()
        Else
            WebMsgBox.Show("Unknown report type requested.")
        End If
        gvItems.DataSource = oDataTableOrDataView
        gvItems.PageSize = CInt(ddlRows.SelectedItem.Text)
        gvItems.DataBind()
        Dim bHasRows As Boolean = False
        If TypeOf oDataTableOrDataView Is DataTable Then
            If oDataTableOrDataView.Rows.Count > 0 Then
                bHasRows = True
            End If
        ElseIf TypeOf oDataTableOrDataView Is DataView Then
            If oDataTableOrDataView.Count > 0 Then
                bHasRows = True
            End If
        End If
        If bHasRows Then
    ...
    Protected Function dvGetProductTotals() As DataView
        'Dim dtConsignments As DataTable = dtGetConsignmentBreakdown()
        Dim dictProducts As New Dictionary(Of Int32, Int32)
        Dim sSelectedCategory As String = String.Empty
        If rbMarketing.Checked Then
            sSelectedCategory = "market"
        ElseIf rbOperations.Checked Then
            sSelectedCategory = "operation"
        ElseIf rbPrepaidCards.Checked Then
            sSelectedCategory = "paid"
        ElseIf rbWUPSUK.Checked Then
            sSelectedCategory = "uk"
        End If

        Dim sSQL As String = "SELECT lp.LogisticProductKey, lp.ProductCategory, lm.ItemsOut 'Qty' FROM LogisticMovement lm INNER JOIN LogisticProduct lp ON lm.LogisticProductKey = lp.LogisticProductKey INNER JOIN Consignment c ON lm.ConsignmentKey = c.[key] WHERE c.StockBookingKey IS NOT NULL AND lm.CustomerKey = " & Session("CustomerKey") & " AND lm.LogisticMovementStartDateTime BETWEEN '" & psFromDate & "' AND '" & psToDate & "' AND NOT (c.StateId ='CANCELLED' OR c.StateId = 'ON_HOLD') ORDER BY lm.LogisticMovementKey"
        Dim dtConsignments As DataTable = ExecuteQueryToDataTable(sSQL)
        For Each drConsignment As DataRow In dtConsignments.Rows
            If drConsignment("ProductCategory").ToString.ToLower.Contains(sSelectedCategory) Then
                Dim nLogisticProductKey As Int32 = drConsignment("LogisticProductKey")
                Dim nQty As Int32 = drConsignment("Qty")
                If dictProducts.ContainsKey(nLogisticProductKey) Then
                    Dim nCurrentValue As Int32 = dictProducts(nLogisticProductKey)
                    dictProducts(nLogisticProductKey) = nCurrentValue + nQty
                Else
                    dictProducts.Add(nLogisticProductKey, nQty)
                End If
            End If
        Next
        Dim dtResult As New DataTable
        dtResult.Columns.Add(New DataColumn("Product Code", GetType(String)))
        dtResult.Columns.Add(New DataColumn("Product Date", GetType(String)))
        dtResult.Columns.Add(New DataColumn("Product Description", GetType(String)))
        dtResult.Columns.Add(New DataColumn("Qty", GetType(Int32)))
        
        Dim drResult As DataRow
        
        For Each kv As KeyValuePair(Of Int32, Int32) In dictProducts
            Dim drProductInfo As DataRow = ExecuteQueryToDataTable("SELECT ProductCode, ProductDate, ProductDescription FROM LogisticProduct WHERE LogisticProductKey = " & kv.Key).Rows(0)
            drResult = dtResult.NewRow
            drResult("Product Code") = drProductInfo("ProductCode")
            drResult("Product Date") = drProductInfo("ProductDate")
            drResult("Product Description") = drProductInfo("ProductDescription")
            drResult("Qty") = kv.Value
            dtResult.Rows.Add(drResult)
        Next
        Dim dv As New DataView(dtResult)
        dv.Sort = "Product Code"
        dvGetProductTotals = dv
    End Function
        
    Protected Function dtGetConsignmentBreakdown() As DataTable
        Dim oConn As New SqlConnection(gsConn)
        Dim oDataTable As New DataTable
        ...
        oAdapter.SelectCommand.Parameters("@IncludeConsignmentCost").Value = 1
        ...
        For Each dr As DataRow In oDataTable.Rows
            ...
            If sProductCategory.ToLower.Contains(sSelectedCategory) Then
    ...
    Protected Sub ExportProductDetails()
        ...
        ElseIf rbConsignments.Checked Then
            'oDataTableOrDataView = dtGetConsignmentBreakdown()
            oDataTableOrDataView = SortItems(dtGetConsignmentBreakdown, isPageIndexChanging:=False)
        Else
            WebMsgBox.Show("Unknown report type requested.")
        End If

        'Dim oDataView As New DataView()
        'oDataView = SortItems(dtGetConsignmentBreakdown, isPageIndexChanging:=False)
        ...
        If oDataTableOrDataView.Count > 0 Then
            ...
            If rbTotals.Checked Then
                sFilename &= " Totals)"
            Else
                sFilename &= " Consignments)"
            End If
            Response.Clear()
            'Response.ContentType = "Application/x-msexcel"
            Response.ContentType = "text/csv"
            Response.AddHeader("Content-Disposition", "attachment; filename=WU Platform Usage (" & sFilename & ".csv")
            ...
            For Each oDataColumn In oDataTableOrDataView.Table.Columns  ' write column header
            ...
            For Each oDataRowView As DataRowView In oDataTableOrDataView
                For Each oDataColumn In oDataTableOrDataView.Table.Columns
                    If Not IgnoredItems.Contains(oDataColumn.ColumnName) Then
    ...
    Protected Sub rbOptionChanged(sender As Object, e As System.EventArgs)
        Call RunReport()
    End Sub
...
<body>
    ...
    <table id="tblDateRangeSelector" runat="server" visible="true" width="100%">
        <tr id="Tr1" runat="server">
            <td colspan="4" style="white-space: nowrap">
                <asp:Label ID="lblPageHeading" runat="server" ForeColor="Silver" Font-Size="Small"
                    Font-Bold="True" Font-Names="Arial">Western Union - Platform Usage Report</asp:Label>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <asp:RadioButton ID="rbMarketing" Font-Names="Verdana" Font-Size="XX-Small" runat="server"
                    Text="Marketing" GroupName="category" Checked="True" AutoPostBack="True" 
                    oncheckedchanged="rbOptionChanged" />
                &nbsp;<asp:RadioButton ID="rbOperations" Font-Names="Verdana" Font-Size="XX-Small"
                    runat="server" Text="Operations" GroupName="category" 
                    AutoPostBack="True" oncheckedchanged="rbOptionChanged" />
                &nbsp;<asp:RadioButton ID="rbPrepaidCards" Font-Names="Verdana" Font-Size="XX-Small"
                    runat="server" Text="Prepaid Cards" GroupName="category" 
                    AutoPostBack="True" oncheckedchanged="rbOptionChanged" />
                &nbsp;<asp:RadioButton ID="rbWUPSUK" Font-Names="Verdana" Font-Size="XX-Small" runat="server"
                    Text="WUPSUK" GroupName="category" AutoPostBack="True" oncheckedchanged="rbOptionChanged" />
            </td>
            <td>
                <asp:RadioButton ID="rbTotals" Font-Names="Verdana" Font-Size="XX-Small" runat="server"
                    Text="Show Per-Product Totals" GroupName="reporttype" Checked="True" 
                    AutoPostBack="True" oncheckedchanged="rbOptionChanged" />
            &nbsp;<asp:RadioButton ID="rbConsignments" Font-Names="Verdana" 
                    Font-Size="XX-Small" runat="server"
                    Text="Show Consignments" GroupName="reporttype" AutoPostBack="True" oncheckedchanged="rbOptionChanged" />
            </td>
            <td>
                &nbsp;
            </td>
    ...
    <asp:Panel ID="pnlData" runat="server" Visible="false" Width="100%">
        <asp:GridView ID="gvItems" runat="server" Width="100%" OnPageIndexChanging="gvItems_PageIndexChanging"
            OnSorting="gvItems_Sorting" AllowPaging="True" AllowSorting="True" CellPadding="2"
            Font-Names="Verdana" Font-Size="XX-Small" OnRowDataBound="gvItems_RowDataBound">
