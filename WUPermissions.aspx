...
<script runat="server">
    ...
    Const USER_PERMISSION_ACCOUNT_HANDLER As Integer = 1                    ' bit 1
    Const USER_PERMISSION_SITE_ADMINISTRATOR As Integer = 2                 ' bit 2
    Const USER_PERMISSION_DEPUTY_SITE_ADMINISTRATOR As Integer = 4          ' bit 3
    Const USER_PERMISSION_SITE_EDITOR As Integer = 8                        ' bit 4

    Const USER_PERMISSION_DEPUTY_SITE_EDITOR As Integer = 16                ' bit 5
    Const USER_PERMISSION_32_NOT_USED As Integer = 32                       ' bit 6
    Const USER_PERMISSION_64_NOT_USED As Integer = 64                       ' bit 7
    Const USER_PERMISSION_128_NOT_USED As Integer = 128                     ' bit 8
    
    Const USER_PERMISSION_256_NOT_USED As Integer = 256                     ' bit 9
    Const USER_PERMISSION_512_NOT_USED As Integer = 512                     ' bit 10
    Const USER_PERMISSION_VIEW_STOCK As Integer = 1024                      ' bit 11
    Const USER_PERMISSION_CREATE_STOCK_BOOKING As Integer = 2048            ' bit 12

    Const USER_PERMISSION_PRINT_ON_DEMAND_TAB As Integer = 4096             ' bit 13
    Const USER_PERMISSION_ADVANCED_PERMISSIONS_TAB As Integer = 8192        ' bit 14
    Const USER_PERMISSION_FILE_UPLOAD_TAB As Integer = 16384                ' bit 15
    Const USER_PERMISSION_WU_INTERNAL_USER As Integer = &H8000              ' bit 16

    Const USER_PERMISSION_WU_CREATE_DELETE_ACCOUNTS As Integer = &H10000    ' bit 17
    Const USER_PERMISSION_WU_RESET_PASSWORDS As Integer = &H20000           ' bit 18
    Const USER_PERMISSION_WU_ACCESS_REPORTS As Integer = &H40000            ' bit 19
    Const USER_PERMISSION_WU_VIEW_STOCK As Integer = &H80000                ' bit 20

    Const USER_PERMISSION_WU_ORDER_STOCK As Integer = &H100000              ' bit 21
    Const USER_PERMISSION_WU_IS_TSE As Integer = &H200000                   ' bit 22
    Const USER_PERMISSION_WU_IS_PILOT_USER As Integer = &H400000            ' bit 23
    Const USER_PERMISSION_WU_IS_SALES As Integer = &H800000                 ' bit 24

    Const USER_PERMISSION_PRODUCT_CREDITS_TAB As Integer = &H1000000        ' bit 25
    ...
    Protected Function IListOfPermissions() As IList(Of Int32)
        ...
        IListOfPermissions.Add(USER_PERMISSION_PRODUCT_CREDITS_TAB)
    End Function
    ...
    Protected Sub PopulateUserGridview()
        Dim sSQL As String = "SELECT [key] 'UserKey', UserID + ' (' + FirstName + ' ' + LastName + ')' UserName, ISNULL(UserPermissions, 0) 'UserPermissions', CustomerAccountCode FROM UserProfile up INNER JOIN Customer c ON up.CustomerKey = c.CustomerKey WHERE UserPermissions > 0 AND up.CustomerKey IN (579, 686, 788, 798, 826) ORDER BY UserID"
    ...
    Protected Function GetUsers(Optional ByVal sFilter As String = "") As DataTable
        GetUsers = Nothing
        Dim sSQL As String = "SELECT [key] 'UserKey',  FirstName + ' ' + LastName + ' (' + UserId + ')' 'UserName' FROM UserProfile WHERE [Status] = 'Active' AND CustomerKey IN (579, 686, 788, 798, 826)"
    ...
    Protected Sub ExtractPermissionsByUserKey(ByVal nUserKey As Int32)
        Dim sSQL As String
        sSQL = "SELECT ISNULL(UserPermissions, 0) 'UserPermissions', UserID + ' (' + FirstName + ' ' + LastName + ') - ' + CustomerAccountCode 'UserName' FROM UserProfile up INNER JOIN Customer c ON up.CustomerKey = c.CustomerKey WHERE [key] = " & nUserKey
        ...
        cbIsProductCreditsAuthoriser.Checked = nUserPermissions And USER_PERMISSION_PRODUCT_CREDITS_TAB
        cbFileUploadTab.Checked = nUserPermissions And USER_PERMISSION_FILE_UPLOAD_TAB
    ...
    Protected Sub SavePermissions(ByVal nUserKey As Int32)
        ...
        End If
        If cbFileUploadTab.Checked Then
            nUserPermissions = (nUserPermissions Or USER_PERMISSION_FILE_UPLOAD_TAB)
        Else
            nUserPermissions = (nUserPermissions And (Not USER_PERMISSION_FILE_UPLOAD_TAB))
        End If
        If cbIsProductCreditsAuthoriser.Checked Then
            nUserPermissions = (nUserPermissions Or USER_PERMISSION_PRODUCT_CREDITS_TAB)
        Else
            nUserPermissions = (nUserPermissions And (Not USER_PERMISSION_PRODUCT_CREDITS_TAB))
        End If
        sSQL = "UPDATE UserProfile SET UserPermissions = " & nUserPermissions & " WHERE CustomerKey IN (579, 686, 788, 798, 826) AND [key] = " & nUserKey
    ...
    Protected Sub SetCheckboxState(ByVal bEnabled As Boolean)
        If bEnabled Then
            ...
            cbIsProductCreditsAuthoriser.Enabled = True
            cbFileUploadTab.Enabled = True
        Else
            ...
            cbIsProductCreditsAuthoriser.Checked = False
            cbIsProductCreditsAuthoriser.Enabled = False
            cbFileUploadTab.Enabled = False
        End If
    End Sub
    ...
    Protected Sub gvUsers_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        ...
        If gvr.RowType = DataControlRowType.DataRow Then
            ...
            End If
            If (nUserPermissions And USER_PERMISSION_FILE_UPLOAD_TAB) > 0 Then
                sbTextPermissions.Append("File upload tab; ")
            End If
    ...
    Protected Sub btnGenerateReport_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        sbText.Append("<hr />")
        Call NewLine(sbText)
        Dim dtUsers As DataTable = ExecuteQueryToDataTable("SELECT UserID + ' (' + FirstName + ' ' + LastName + ')' 'User', UserPermissions FROM UserProfile up WHERE up.CustomerKey IN (579, 686, 798, 826) AND ISNULL(UserPermissions,0) > 0 ORDER BY UserID")
        ...
        For Each drUser As DataRow In dtUsers.Rows
            ...
            sbText.Append(" (")
            sbText.Append(drUser("UserPermissions").ToString)
            sbText.Append(")")
            Call NewLine(sbText)
            ...
            End If
            If (nUserPermissions And USER_PERMISSION_PRODUCT_CREDITS_TAB) > 0 Then
                sbText.Append("Is Product Credits Authoriser, ")
            End If
            If (nUserPermissions And USER_PERMISSION_WU_IS_PILOT_USER) > 0 Then
        ...
        For Each nPermission As Int32 In IListOfPermissions()
            ...
            Select Case nPermission
                ...
                Case USER_PERMISSION_PRODUCT_CREDITS_TAB
                    sbText.Append(Bold("Is Product Credits Authoriser"))
        ...
        <table style="width: 100%">
            ...
            <tr id="trModifyPermissions2a" runat="server" visible="true">
                ...
                <td colspan="3">
                    ...
                    &nbsp;<asp:CheckBox ID="cbIsProductCreditsAuthoriser" runat="server" Font-Names="Arial" Font-Size="XX-Small" Text="Is Product Credits Authoriser" />
                    &nbsp;<asp:CheckBox ID="cbFileUploadTab" runat="server" Font-Names="Arial" Font-Size="XX-Small" Text="File Upload Tab" />
                </td>
                <td>
                    &nbsp;
                </td>
                ...
                <td colspan="3">
                    <asp:GridView ID="gvUsers" runat="server" AutoGenerateColumns="False" CellPadding="2"
                        ...
                        <Columns>
                            <asp:TemplateField>
                                <ItemTemplate>
                                    <asp:Button ID="btnEdit" runat="server" CommandArgument='<%# Container.DataItem("UserKey")%>'
                                        OnClick="btnEdit_Click" Text="edit" Width="70px" />
                                </ItemTemplate>
                            ...
                            <asp:BoundField DataField="CustomerAccountCode" HeaderText="Cust Acct" 
                                ReadOnly="True" SortExpression="CustomerAccountCode">
